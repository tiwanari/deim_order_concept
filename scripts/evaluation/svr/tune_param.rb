...
output_path = "#{path_to_eval_data}/svr"
...
Parallel.each(0...num_of_data, in_processes: 8) do |i|
    ...
    c_params.each do |c_value|
        ...
        # Usage: $0 train_test_output_path svr_path
        #           [c_value (default: 0.01)] [type (default: 11)] [ablation (default: 0 = do not)]
        ...
        svr_rank_output = "#{tune_data_sub_path}/eval/svr_result_c#{c_value}_t#{type}.csv"
        FileUtils.mkdir_p(File.dirname(svr_rank_output))

        STDERR.puts "./concat_svr_results.rb #{tune_data_sub_path} #{svr_rank_output}"
        puts_line("concate_svr_results", OUT_LENGTH, STDERR)
        `./concat_svr_results.rb #{tune_data_sub_path} #{svr_rank_output}`


        # create test gold (N - 1) results
        test_gold = gold_standard - [gold_standard[i]]

        # make some temporary files in #{path_to_eval_data}/svr/tmp/
        ...
        # e.g. #{__FILE__} ./gold/gold0.csv ../../result/gather/ver1/svr_result.txt
